...
var shangJia = true
function shangjia() {
    $(".shangjia").attr("src", "image/shangjia.png")
    $(".weishangjia").attr("src", "image/weishangjia.png")
    shangJia = true
}
function weishangjia() {
    $(".weishangjia").attr("src", "image/shangjia.png")
    $(".shangjia").attr("src", "image/weishangjia.png")
    shangJia = false
}
...
if (getCookie("ProductID")) {
    ...
    function getDetail() {
        $.ajax({
            type: "post",
            url: mainurl + "api/P_Mall/ProductDetail?ID=" + getCookie("ProductID"),
            dataType: "json",
            async: true,
            success: function (data) {
                if (data.Status == 1) {
                    ...
                    if (data.Result.EDetail != null) {
                        ue2 = UE.getEditor('container2');
                        detail2 = data.Result.EDetail
                        ue2.ready(function () {
                            this.setContent(decodeURIComponent(data.Result.EDetail));
                        })
                    } else {
                        ue2 = UE.getEditor('container2');
                        ue2.ready(function () {
